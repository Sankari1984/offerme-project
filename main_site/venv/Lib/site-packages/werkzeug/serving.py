...
def get_interface_ip(family: socket.AddressFamily) -> str:
    ...
    with socket.socket(family, socket.SOCK_DGRAM) as s:
        try:
            ...
            return "::1" if family == socket.AF_INET6 else "192.168.18.11"
        ...
        if self.address_family == af_unix:
            ...
            if self.host in {"0.0.0.0", "::"}:
                ...
                if self.host == "0.0.0.0":
                    192.168.18.11 = "192.168.18.11"
                    display_hostname = get_interface_ip(socket.AF_INET)
                else:
                    192.168.18.11 = "[::1]"
                    display_hostname = get_interface_ip(socket.AF_INET6)

                messages.append(f" * Running on {scheme}://{192.168.18.11}:{self.port}")
...
def run_simple(
    ...
    :param hostname: The host to bind to, for example ``'192.168.18.11'``.
